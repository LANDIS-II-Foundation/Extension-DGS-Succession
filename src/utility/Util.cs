...
namespace Landis.Extension.Succession.DGS
{
    ...
    public static class Util
    {
        ...
        public static void ReadSoilCMap(string path)
        {
            IInputRaster<DoublePixel> map = MakeDoubleMap(path);
            ...
            using (map)
            {
                ...
                foreach (Site site in PlugIn.ModelCore.Landscape.AllSites)
                {
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 90000.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "SOM Carbon {0} is not between {1:0.0} and {2:0.0}. Site_Row={3:0}, Site_Column={4:0}",
                                                          mapValue, 0.0, 90000.0, site.Location.Row, site.Location.Column);
        ...
        public static void ReadSoilNMap(string path)
        {
            IInputRaster<DoublePixel> map = MakeDoubleMap(path);
                ...
                foreach (Site site in PlugIn.ModelCore.Landscape.AllSites)
                {
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 5000.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "SOM Nitrogen value {0} is not between {1:0.0} and {2:0.0}. Site_Row={3:0}, Site_Column={4:0}",
